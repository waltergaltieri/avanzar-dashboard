...
function keyValuePairsToObjectUnmapper(value) {
    ...
    const propertyDescriptors = (0, globals_1.safeMap)(safeReflectOwnKeys(value), (key) => [
        key,
        safeObjectGetOwnPropertyDescriptor(value, key),
    ]);
    if (!(0, globals_1.safeEvery)(propertyDescriptors, ([, descriptor]) => isValidPropertyNameFilter(descriptor))) {
        throw new globals_1.Error('Incompatible instance received: should contain only c/e/w properties without get/set');
    }
    return [(0, globals_1.safeMap)(propertyDescriptors, ([key, descriptor]) => [key, descriptor.value]), hasNullPrototype];
}
